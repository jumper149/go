...
handler path config = gameH :<|> wssH :<|> publicH
  where gameH :: Monad m => m GameHtml
        ...
        wssH = do gs <- asks gsMVar
                  return $ websocketsOr defaultConnectionOptions (handler gs) backupApp
          where handler :: MVar (GameState b c n) -> PendingConnection -> IO ()
                handler gsMVar pendingConn = do conn <- acceptRequest pendingConn
                                                gs <- readMVar gsMVar
...
server port path = do putStrLn $ "Port is: " <> show port
                      ...
                      let app = serve api $ hoistServer api (runHandlerM $ ServerState gs) (handler path config)
                      run port app
